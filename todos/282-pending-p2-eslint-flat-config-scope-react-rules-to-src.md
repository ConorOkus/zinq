...
Commit `3b33e19` re-included `api/**/*.ts` in eslint by removing it from ignores. The existing eslint flat-config block targets `**/*.{ts,tsx}` and applies React-flavored plugins (`react`, `react-hooks`, `react-refresh`, `jsx-a11y`) to _every_ TS file — including the three Vercel serverless handlers under `api/`. The handlers are non-JSX Node code; React rules are wasted work and risk false positives (notably `react-refresh/only-export-components`, since Vercel handlers _are_ default exports).
